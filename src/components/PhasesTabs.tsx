...
import { ArrowRight, Gavel, Hammer, Search } from 'lucide-react';
...
export default function PhasesTabs() {
  ...
  // Efecto para la navegación pegajosa (Sticky) al hacer scroll
  useEffect(() => {
    const handleScroll = () => {
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 mb-24 transition-all duration-700 ${isShrunk ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          ...
          <button 
            ...
            className={`relative p-10 rounded-2xl transition-all duration-500 text-center flex flex-col items-center border-2 ${activeTab === 'fase1' ? 'bg-slate-900/60 border-cyan-500 shadow-[0_0_40px_rgba(34,211,238,0.15)]' : 'bg-slate-900/40 border-slate-800 hover:border-slate-700'}`}
          >
            ...
            <p className="text-slate-400 text-[11px] font-bold uppercase leading-relaxed tracking-wider">Protocolo <span className="text-white">PCF-15™</span> para asegurar tu inversión. Si ya elegiste tu propiedad, la auditamos; si no, activamos modo sourcing.</p>
          </button>

          {/* FASE 2 */}
          <button 
            onClick={() => handleTabChange('fase2')}
            className={`relative p-10 rounded-2xl transition-all duration-500 text-center flex flex-col items-center border-2 ${activeTab === 'fase2' ? 'bg-slate-900/60 border-cyan-500 shadow-[0_0_40px_rgba(34,211,238,0.15)]' : 'bg-slate-900/40 border-slate-800 hover:border-slate-700'}`}
          >
            ...
            <p className="text-slate-400 text-[11px] font-bold uppercase leading-relaxed tracking-wider">Inteligencia de mercado y valorización de fallas <span className="text-white">PCF-15 (NDI)</span> para generar poder de negociación.</p>
          </button>

          {/* FASE 3 */}
          <button 
            onClick={() => handleTabChange('fase3')}
            className={`relative p-10 rounded-2xl transition-all duration-500 text-center flex flex-col items-center border-2 ${activeTab === 'fase3' ? 'bg-slate-900/60 border-cyan-500 shadow-[0_0_40px_rgba(34,211,238,0.15)]' : 'bg-slate-900/40 border-slate-800 hover:border-slate-700'}`}
          >
            ...
            <p className="text-slate-400 text-[11px] font-bold uppercase leading-relaxed tracking-wider">Ejecución de precisión para corregir fallas de auditoría, garantizando habitabilidad y plusvalía inmediata.</p>
          </button>
        </div>

        {/* NAVEGACIÓN STICKY - SOLO VISIBLE AL HACER SCROLL */}
...
// COMPONENTE DE TRANSICIÓN AL FINAL DE CADA FASE
  ...
  return (
    <div className="pt-24 border-t border-slate-900/50">
      ...
      <button
        ...
        className="w-full max-w-4xl mx-auto p-10 rounded-[2rem] border-2 border-slate-800 bg-slate-900/30 hover:border-cyan-500 hover:bg-cyan-500/5 transition-all group overflow-hidden relative"
      >
