...
// --- HOJA 1: EL GANCHO DE NEGOCIOS (INTERLINEADO COMPACTO) ---
export function HeroHook({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  ...
  return (
    <motion.div 
      ...
      className="absolute inset-0 z-10 flex flex-col justify-between px-6 max-w-7xl mx-auto h-screen py-16 md:py-20 pointer-events-none"
    >
      {/* TEXTO SUPERIOR: INTERLINEADO REDUCIDO EN MÓVIL (leading-[0.95]) */}
      <div className="text-left relative z-10 pointer-events-auto max-w-3xl mt-10">
        <h1 className="text-5xl md:text-7xl font-black text-white leading-[0.95] md:leading-tight mb-4 uppercase tracking-tighter">
          NO COMPRES <br />
        ...
        <div className="mb-6">
          <p className="text-4xl md:text-6xl font-black text-[#F59E0B] leading-[0.95] md:leading-none mb-4 uppercase italic">
            AHORRA MILLONES
          </p>
...
// --- HOJA 2: INFORME TÉCNICO (CON ESPACIADO PARA SEGUNDA HOJA) ---
export function HeroSpecs({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  ...
  const x = useTransform(scrollYProgress, [0.8, 0.95], [50, 0]);
  ...
  return (
    ...
    >
      {/* Margen superior alto (mt-20) para asegurar que no se traslape con Hoja 1 */}
      <div className="bg-slate-950/80 backdrop-blur-md p-8 border-l-4 border-[#F59E0B] rounded-r-lg shadow-2xl pointer-events-auto max-w-xl w-full mt-20">
