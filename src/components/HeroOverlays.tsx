...
// --- HOJA 1: GANCHO DE NEGOCIOS (ESPACIADO RESPONSIVO DIFERENCIADO) ---
export function HeroHook({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  ...
  return (
    ...
    >
      {/* TITULAR: mb-1 para celular | md:mb-10 para la web (escritorio) */}
      <div className="text-left relative z-10 pointer-events-auto max-w-3xl mt-4 md:mt-10">
        <h1 className="text-5xl md:text-7xl font-black text-white leading-[0.85] mb-1 md:mb-10 uppercase tracking-tighter">
          NO COMPRES <br />
        ...
        <div className="mb-6">
          {/* BOX HIGHLIGHT: SE PEGA AL TITULAR SOLO EN CELULAR */}
          <div className="inline-block bg-slate-950/70 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/5 mb-4">
            <p className="text-4xl md:text-6xl font-black text-green-500 leading-none uppercase italic">
              AHORRA MILLONES
            </p>
          </div>
      ...
      {/* TARJETA DE ACCIÓN: ELEVADA EN CELULAR (mb-32) */}
      <div className="flex justify-end items-end w-full mb-32 md:mb-0">
        <div className="bg-slate-950/90 backdrop-blur-xl p-8 border border-white/10 rounded-2xl shadow-2xl pointer-events-auto max-w-sm w-full relative z-20">
          ...
          <button onClick={handleWhatsAppClick} className="w-full bg-[#F59E0B] hover:bg-[#d98c08] text-slate-950 font-black py-4 rounded-xl uppercase transition-all flex items-center justify-center gap-2 group text-[13px] leading-tight">
