      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-2xl md:text-3xl text-slate-300 font-light uppercase tracking-wide">
            Tu tiempo es <span className="text-cyan-400 font-bold italic underline decoration-cyan-400/30">Dinero</span>
          </p>
        </div>
        ...
        {/* LEYENDA PIE DE PÁGINA (TAMAÑO CORREGIDO) */}
        <div className="max-w-4xl mx-auto text-center space-y-6 pt-12 border-t border-slate-900">
          <p className="text-lg md:text-xl text-slate-400 font-medium italic leading-relaxed">
            "Coordinamos visitas para asistir en conjunto, pero si tu agenda se complica, <span className="text-white font-black not-italic">DOMIS™ no se detiene.</span>"
          </p>
          <div className="flex flex-col items-center">
            <span className="text-2xl md:text-4xl font-black text-white uppercase tracking-[0.4em]">
              VAMOS POR <span className="text-cyan-500">TI</span>
            </span>
            <div className="w-16 h-1 bg-cyan-500 mt-4 rounded-full"></div>
            <Clock size={16} className="text-slate-800 mt-4" />
          </div>
        </div>
      </div>
    </section>
...
function SourcingNormal({ onSelect }: { onSelect: any }) {
  ...
  return (
    <div className="bg-slate-900/40 border border-white/5 p-10 rounded-[2.5rem] hover:border-slate-700 transition-all flex flex-col group relative">
      ...
      <p className="text-slate-400 text-sm mb-8 leading-relaxed">
        Barremos la red por ti y depuramos las opciones digitalmente. Te entregamos una selección de élite para que tú decidas qué auditamos a fondo.
      </p>
      ...
      <div className="bg-slate-950/80 rounded-[2rem] p-8 border border-slate-800 mb-8 mt-auto">
        <div className="space-y-6">
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setCantidad(3)} className={`flex-1 py-3 rounded-xl font-black border-2 ${cantidad === 3 ? 'bg-white text-slate-950 border-white' : 'border-slate-800 text-slate-500'}`}>
                {cantidad === 3 ? <input type="number" value={cantidadCustom} onChange={e => setCantidadCustom(Math.max(3, Number(e.target.value)))} className="w-full bg-transparent text-center outline-none" min="3"/> : '3+'}
              </button>
            </div>
          ...
          <div>
            ...
            <input type="number" value={meters} onChange={e => setMeters(Math.max(100, Number(e.target.value)))} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white font-mono" />
          </div>
          <div className="pt-4 border-t border-slate-800 flex justify-between items-center text-cyan-400 font-mono font-black text-xl">
            <span className="text-xs font-black text-slate-500 uppercase flex items-center gap-1"><Info size={10} /> Total:</span>
            <span>${total.toLocaleString()} + IVA</span>
          </div>
        </div>
      </div>
      <button onClick={() => onSelect('Normal', efectiveCantidad, effectiveMeters, total)} className="w-full bg-slate-800 hover:bg-white hover:text-slate-950 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-2">
...
function SourcingVIP({ onSelect }: { onSelect: any }) {
  ...
  return (
    <div className="bg-gradient-to-b from-slate-900 to-slate-950 border-2 border-cyan-500/40 p-10 rounded-[2.5rem] relative overflow-hidden flex flex-col shadow-2xl group">
      ...
      <p className="text-slate-400 text-sm mb-8 leading-relaxed">
        Evaluamos en terreno cada opción que te entregamos. Recibes un <span className="text-white font-bold italic">Informe FAST</span> para comparar estados reales y decides con total certeza cuáles propiedades pasan a Auditoría Full.
      </p>

      <div className="bg-slate-950/80 rounded-[2rem] p-8 border border-cyan-900/30 mb-8 mt-auto shadow-inner">
        <div className="space-y-6">
          <div>
            <label className="text-[10px] font-black uppercase text-cyan-500 mb-4 block tracking-widest flex items-center gap-1"><MapPin size={10} className="inline mr-1" /> Pack VIP:</label>
            <div className="flex gap-2">
            ...
            <div className="bg-cyan-500/5 border border-cyan-500/20 rounded-xl p-4 mt-4 space-y-2 text-[10px] font-bold uppercase tracking-wider text-slate-300">
              <div className="flex items-center gap-2 text-white"><CheckCircle size={12} className="text-cyan-400" /><span>Visitamos con FAST: <strong className="text-cyan-400">{labels} propiedades</strong></span></div>
          ...
          <div>
            ...
            <input type="number" value={meters} onChange={e => setMeters(Math.max(100, Number(e.target.value)))} className="w-full bg-slate-800 border border-cyan-900/30 rounded-xl px-4 py-3 text-white font-mono" />
          </div>
      ...
      <button onClick={() => onSelect('VIP', efectiveCantidad, effectiveMeters, total)} className="w-full bg-cyan-500 hover:bg-white text-slate-950 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] transition-all shadow-[0_0_30px_rgba(34,211,238,0.3)] flex items-center justify-center gap-2">
